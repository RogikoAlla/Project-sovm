...
2. Держатель **Короля пик** получает роль **King**; остальные роли — Ace, Queen, Servant — назначаются **против часовой стрелки**.
3. Цель: оставаться King как можно дольше; остальные стремятся занять эту роль.
4. **King** объявляет козырную масть в начале раунда.
5. Атака идёт по часовой: Servant → Queen → Ace → King.
6. Атакующий может **подкидывать** карты того же ранга, что уже на столе; только атакующий подкидывает или объявляет отбой.
7. Если защитник **берёт** карты со стола, роли атакующего и защитника **меняются** (исключение: Servant берёт у King — без смены).
8. **King** один раз за раунд может **слепо обменять** руку с любым игроком.
...
├── tests/           # pytest (≥90% coverage на common/server)
...
```bash
python -m server.main --deck 36
```

### Клиент (4 терминала)

```bash
python -m client.main --host 127.0.0.1 --name Alice
```
